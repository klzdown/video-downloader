// -------------------------
// SCRIPT.JS (downloader + lightning)
// -------------------------
// CONFIGURATION
...
let statusBox = document.getElementById("statusBox"); // may be null initially
...
// --- STATUS helpers (inject if missing) ---
...
function showStatus(msg, kind = "info") {
  ...
  if (!box) { try { alert(msg); } catch (e) {} return; }
  box.dataset.type = kind;
  box.textContent = msg;
  box.style.display = "block";

  if (kind === "error") { box.style.background = "rgba(180,40,60,0.95)"; box.style.color = "#fff"; }
  else if (kind === "success") { box.style.background = "rgba(30,120,60,0.95)"; box.style.color = "#fff"; }
  else { box.style.background = "rgba(30,60,90,0.95)"; box.style.color = "#eaf2ff"; }
...
// util collectUrls + pickThumbnail
function collectUrls(obj, out = new Set()) {
  if (!obj) return out;
  if (typeof obj === "string") { const s = obj.trim(); if (/^https?:\/\//i.test(s)) out.add(s); return out; }
...
async function callApi(videoUrl) {
  ...
  if (!res.ok) {
    ...
    const err = new Error(`HTTP ${res.status}`); err.raw = text; throw err;
  }
  const ct = res.headers.get("content-type") || "";
  if (ct.includes("application/json")||ct.includes("text/json")) return res.json();
  const txt = await res.text();
  try { return JSON.parse(txt); } catch(e) { const err = new Error("Upstream returned non-JSON response"); err.raw = txt; throw err; }
}
...
async function downloadBlob(url, filename = "video.mp4") {
  try {
    ...
    if (String(msg).toLowerCase().includes("cors")) msg = "Gagal mendownload — kemungkinan diblokir CORS. Gunakan server-proxy untuk mengatasi.";
    showStatus("Error: " + msg, "error");
    throw err;
  }
}

// renderResult (sama seperti versi sebelumnya, sedikit disederhanakan)
function renderResult(payload) {
  ...
  for (const d of downloads) { if (d.url && (/\.mp4(\?|$)/i.test(d.url) || /\/play\/|\/video\//i.test(d.url))) { playableUrl = d.url; break; } }
...
if (clearBtn) {
  clearBtn.addEventListener("click", () => { if (urlInput) urlInput.value = ""; clearResults(); });
}
  ...
  if (!window.__downloadHandlerInstalled) {
    ...
    resultList.addEventListener("click", async (e) => {
      ...
      if (!urlInput || !urlInput.value.trim()) {
        showStatus("Masukkan URL video dulu.", "error"); return;
      }
...
async function forceDownloadVideo(url) {
  try {
    ...
    if (String(msg).toLowerCase().includes("cors")) msg = "Gagal download video — kemungkinan diblokir CORS.";
...
(function initLightning() {
  ...
  // spawn a flash object
  function spawnFlash(x = Math.random()*w, y = Math.random()*h*0.6) {
    const f = {
      x, y,
    ...
    // create 2-5 branches each branch is an array of points
    const bcount = 2 + Math.floor(Math.random()*4);
    for (let i=0;i<bcount;i++) {
      ...
      for (let s=0;s<seg;s++) {
        const ang = angleBase + (Math.random()-0.5) * 1.2;
        const len = 20 + Math.random()*60;
        px += Math.cos(ang)*len;
        py += Math.sin(ang)*len + (s*2);
        pts.push({x:px, y:py});
      }
      f.branches.push(pts);
    }
  ...
  function maybeSpawn(ts) {
    // rare spontaneous spawns
    if (Math.random() < 0.02) spawnFlash(Math.random()*w, Math.random()*h*0.45);
  }

  function drawFlash(f, age) {
    const t = age / f.life;
    const alpha = Math.max(0, 1 - t) * f.intensity;
    // glow background (big soft fill)
    ctx.save();
    ctx.globalCompositeOperation = "lighter";
    for (let bi=0; bi<f.branches.length; bi++) {
      const pts = f.branches[bi];
      ctx.beginPath();
      ctx.moveTo(f.x, f.y);
      for (let i=0;i<pts.length;i++) ctx.lineTo(pts[i].x, pts[i].y);
      ctx.lineWidth = 10 * (1 - t);
      ctx.strokeStyle = `rgba(120,190,255,${alpha*0.12})`;
      ctx.shadowColor = `rgba(120,200,255,${alpha*0.25})`;
      ctx.shadowBlur = 30;
      ctx.stroke();
    }
    // main bright stroke
    ...
    // occasional flash overlay (a quick screen-wide flash)
    if (Math.random() < 0.06 && t < 0.2) {
      ctx.save();
      ctx.globalCompositeOperation = "screen";
      ctx.fillStyle = `rgba(200,230,255,${0.05 * (1-t) * f.intensity})`;
  ...
  function frame(ts) {
    const dt = ts - lastTs;
    lastTs = ts;
    // fade canvas a bit to keep trails (semi-transparent rect)
    ctx.clearRect(0,0,w,h);
    // subtle dimming can be omitted; we rely on flashes
    maybeSpawn(ts);
  ...
  // initial few flashes
  for (let i=0;i<3;i++) setTimeout(()=>spawnFlash(Math.random()*w, Math.random()*h*0.45), i*300);

  // expose manual trigger for dev/testing
  window.__triggerLightning = spawnFlash;

  // cleanup on unload
  window.addEventListener("unload", ()=>{ if (animReq) cancelAnimationFrame(animReq); });
})();
